...
class CRnum(CR):
    ...
    def __init__(self, value):
        if not isinstance(value, sympy.Symbol): self.value = sympy.S(str(value), rational=True)
...
class CRsum(CR):
    def simplify(self):
        j = len(self)-1
        while j > 0 and isinstance(self[j], CRnum) and self[j].is_zero(): j -= 1
        if j == 0: return CRnum(0)
...
class CRprod(CR):
    def simplify(self):
        ...
        for i in range(len(self)):
            if isinstance(self[i], CRnum) and self[i].is_zero(): break
        while i > 0 and isinstance(self[i], CRnum) and self[i].is_one(): i -= 1
        if i == 0: return CRnum(1)
