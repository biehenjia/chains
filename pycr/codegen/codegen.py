...
def _op_val(builder, op, work):
    if isinstance(op, CRnum):
        return ir.Constant(f64, float(op.valueof()))
    return builder.load(work[op.start])


# TODO: put CREconnector logic in here
...
def emit_shift(builder, node, work):
    # CREconnector references another CR's slot; shift is a no-op
    if isinstance(node, CREconnector):
        return
    ...
    elif isinstance(node, CRE): emit_cre(builder, node, work)
